...
- **Holds:** design-system components, feature screens, **stores** (`src/<app>/store`),
  DI wiring for React (`src/{design-system,example,di}`).
...
- **Pattern (one-way flow — see [flows.md](../../docs/ai/flows.md)):**
  ...
  - A **store** (Zustand) is a thin reactive cache + dispatch; each action just
    ...
    result. Build it from the DI bundles in `src/<app>/store/hooks.ts` (the ONLY
