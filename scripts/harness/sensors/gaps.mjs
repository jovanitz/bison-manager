...
const sourceFiles = allFiles.filter((f) => !SPEC_EXT.test(f) && !/test-setup\./.test(f));
...
for (const spec of specFiles) {
  ...
  while ((m = IMPORT_RE.exec(src))) {
    ...
    for (const c of candidatesFor(resolved)) if (existsSync(c)) tested.add(path.resolve(c));
  }
}
...
const add = (type, severity, file, message, suggestion) =>
  gaps.push({ type, severity, layer: layerOf(file), file: rel(file), message, suggestion });

const ADAPTER_DIR = /\/(?:infrastructure\/src\/(?:api|persistence|sync|auth)|platform\/src\/(?:browser|capacitor|tauri))\//;

for (const f of sourceFiles) {
  const r = rel(f);
  const base = path.basename(f);
  if (base.startsWith('index.') || base === 'errors.ts' || base === 'dto.ts') continue;
  ...
  if (
    ...
    if (/export\s+(?:const|function)\s+create/.test(content) && !isExercised(f)) {
      add(
        'untested-adapter',
        'high',
        f,
...
for (const f of sourceFiles) {
  ...
  lines.forEach((line, i) => {
    if (TODO_RE.test(line)) {
      add('todo', 'low', f, `Marker at line ${i + 1}: ${line.trim().slice(0, 120)}`, 'Resolve or convert to a tracked task.');
    }
  });
}

const filtered = LAYER_FILTER ? gaps.filter((g) => g.layer === LAYER_FILTER) : gaps;
const countBy = (key) =>
  filtered.reduce((acc, g) => ((acc[g[key]] = (acc[g[key]] || 0) + 1), acc), {});

const report = {
  tool: 'gaps',
  generatedAt: new Date().toISOString(),
  ok: filtered.filter((g) => g.severity === 'high').length === 0,
  summary: { total: filtered.length, byType: countBy('type'), bySeverity: countBy('severity') },
